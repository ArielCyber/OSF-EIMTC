...
class ModelWrapper(tf.keras.Model):
    ...
    def __init__(self,inner_model,name,input_shape=None,deep_model=None,units=128,dropout_rate=0.2):
        super(ModelWrapper, self).__init__(name=name)
    ...
    def fit(self,x=None,y=None,**kwargs):
        ...
        return super(ModelWrapper,self).fit(inner_model_x,np.stack(y),**kwargs)
    ...
    def predict(self,x,**kwargs):
        if self.is_classifer:
            ...
            inner_model_x = self.fit_wrapper_model(x)
        return super(ModelWrapper,self).predict(inner_model_x,**kwargs)